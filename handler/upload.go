...
import (
	...
	"cloudstorage-server/meta"
	"cloudstorage-server/mq"
...
func UploadHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method == "GET" {
		...
		if cfg.CurrentStoreType == cmn.StoreCeph {
			...
			cephPuterror := ceph.PutObject("userfile", "/"+fileMeta.FileSha1, data)
			if cephPuterror != nil {
				fmt.Println(cephPuterror.Error())
				return
			}
...
func DownloadURLHandler(w http.ResponseWriter, r *http.Request) {
	...
	if strings.HasPrefix(row.FileAddr.String, "/tmp") {
		...
		w.Write([]byte(cephUrl))
		//ceph.DownloadCephData("userfile", filehash, w)
